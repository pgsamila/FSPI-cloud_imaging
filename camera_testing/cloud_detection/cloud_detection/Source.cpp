...
int main (void)
{
	...
	vcap.open("http://192.168.137.64:8080/video"); // ip address of the video stream
	...
	while(1)
	{
		...
		int x = 0;
		if (!took_first_frame) // take the first refference frame
		{
			while (x < 1)
			{
				b_video_play = vcap.read(first_frame);
				if (!b_video_play)
				{
		...
		b_video_play = vcap.read(frame); // read a new frame from video
